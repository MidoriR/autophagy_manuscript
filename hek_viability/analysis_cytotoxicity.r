data<-read.table('hek_viability.txt', header=T)
library(plyr)
library(ggplot2)
...
ggsave(surv_plot, file = 'HEKs_survival.pdf')

# Anova test for determining the differences in the treatments and control

aov_cond <- aov(data$Percentage.alive ~ data$Condition)

print(summary(aov_cond))

# Compare all means to determine differences among conditions
